...
- Handles ICE, offer/answer exchange, candidate buffering, per-peer data channels, and basic state sync.
...
Sender:
- constructor(config: { driver, iceServers?, verify?, connectionTimeout?, queueSize?, audioBitrate?, videoBitrate? })
- start({ room?, stream?, state?, channels? })
- stop()
- send(data, { peer?, channel? })
- sync(state, { merge? })
...
Events: `stream`, `connect`, `dispose`, `sync`, `channel:open`, `channel:close`, `channel:error`, `channel:message`
